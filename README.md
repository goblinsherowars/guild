...
The Hydra guide (`hydra.html`) loads the guild-provided instructions from `content/hydra/ru.html` or `content/hydra/en.html`. Edit these files to update the guide. Core lineups use `data-hero-lineup` with space-separated hero IDs and render catalog portraits through the shared hero-team component. The page follows the RU/EN switch and is linked from the navigation and Home page.
